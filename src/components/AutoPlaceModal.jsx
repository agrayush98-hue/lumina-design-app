...
// ── Tiny helpers ──────────────────────────────────────────────────────────────
function Tag({ children, color = '#2d4f68' }) {
  return (
    <span style={{
...
function StepDot({ n, label, active, done }) {
  const col = done ? '#3dba74' : active ? '#d4a843' : '#2d4f68'
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <div style={{
        width: 22, height: 22, borderRadius: '50%', display: 'flex',
        alignItems: 'center', justifyContent: 'center',
        background: done ? '#0e2018' : active ? '#1e1800' : '#0a0f14',
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
          ...
          <span style={{ ...mono, fontSize: 7, color: '#2d4f68', padding: '1px 5px', border: '1px solid #1a2b3c', borderRadius: 3 }}>
...
function StepSpace({ selected, onSelect }) {
  ...
  return (
    <div>
      <div style={{ ...mono, fontSize: 9, color: '#4a7a96', marginBottom: 14, letterSpacing: '0.1em' }}>
        SELECT SPACE TYPE — we'll design a full lighting scheme for it
      </div>
      {categories.map(cat => (
        <div key={cat} style={{ marginBottom: 16 }}>
          <div style={{ ...mono, fontSize: 8, color: '#2d4f68', letterSpacing: '0.12em', marginBottom: 8 }}>
            {cat.toUpperCase()}
          </div>
            ...
            {SPACE_TYPES.filter(s => s.category === cat).map(s => {
              ...
              return (
                <div key={s.id} onClick={() => onSelect(s.id)} style={{
                  ...
                  background: isSel ? '#0e1e2e' : '#080c10',
                  border: `1px solid ${isSel ? '#d4a843' : '#1a2b3c'}`,
                  ...
                  <div style={{ ...mono, fontSize: 7, color: '#2d4f68' }}>{s.targetLux} lx · {s.cct}</div>
                </div>
              )
            })}
          </div>
      ...
      {selected && (() => {
        ...
        return (
          <div style={{
            ...
            background: '#060a0e', border: '1px solid #1a3040',
          }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {space.tags.map(t => <Tag key={t} color='#4a7a96'>{t}</Tag>)}
...
function StepRoom({ space, roomW, roomL, ceilH, onChangeW, onChangeL, onChangeCeilH }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: 24 }}>
      {/* Inputs */}
      <div>
        <div style={{ ...mono, fontSize: 9, color: '#4a7a96', marginBottom: 16, letterSpacing: '0.1em' }}>
          ROOM DIMENSIONS
        </div>
        {[
          ...
          <div key={label} style={{ marginBottom: 14 }}>
            <div style={{ ...mono, fontSize: 8, color: '#2d4f68', marginBottom: 5, letterSpacing: '0.08em' }}>{label}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <input
                ...
                style={{
                  ...
                  border: '1px solid #1a2b3c', borderRadius: 3, padding: '6px 10px',
                }}
              />
              <span style={{ ...mono, fontSize: 10, color: '#4a7a96' }}>{unit}</span>
            </div>
          </div>
        ))}
        ...
        <div style={{
          ...
          background: '#0a0f14', border: '1px solid #1a2b3c',
        }}>
          <div style={{ ...mono, fontSize: 8, color: '#2d4f68', marginBottom: 6, letterSpacing: '0.08em' }}>ROOM SUMMARY</div>
          {[
            ...
            <div key={k} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
              <span style={{ ...mono, fontSize: 8, color: '#2d4f68' }}>{k}</span>
      ...
      <div>
        ...
        <div style={{
          ...
          <div style={{ ...mono, fontSize: 8, color: '#2d4f68', marginBottom: 6, letterSpacing: '0.08em' }}>WHY THIS SCHEME?</div>
    ...
    <div>
      ...
      <div style={{
        ...
        {[
          ...
          { label: 'TARGET LUX',       val: `${space.targetLux}`, unit: 'lx', color: '#4a7a96' },
          { label: 'TOTAL LOAD',       val: `${totalW}`, unit: 'W', color: '#6ae5ff' },
          { label: 'COVERAGE',         val: `${area.toFixed(1)}`, unit: 'm²', color: '#a78bfa' },
        ].map(({ label, val, unit, color }) => (
          <div key={label}>
            <div style={{ ...mono, fontSize: 7, color: '#2d4f68', letterSpacing: '0.1em', marginBottom: 4 }}>{label}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        ...
        <span style={{ ...mono, fontSize: 9, color: '#2d4f68' }}>
      ...
      {/* Layer breakdown */}
      <div style={{ ...mono, fontSize: 9, color: '#4a7a96', marginBottom: 12, letterSpacing: '0.1em' }}>
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ ...mono, fontSize: 14, fontWeight: 700, color: '#cdd9e5' }}>
                  {isStrip ? `${layer.lenM?.toFixed(1)}m` : layer.count}
                </div>
                <div style={{ ...mono, fontSize: 7, color: '#2d4f68' }}>{isStrip ? 'strip' : 'fixtures'}</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ ...mono, fontSize: 14, fontWeight: 700, color: '#6ae5ff' }}>~{luxContrib}</div>
                <div style={{ ...mono, fontSize: 7, color: '#2d4f68' }}>lx contrib</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ ...mono, fontSize: 14, fontWeight: 700, color: '#a78bfa' }}>{Math.round(layer.totalW)}W</div>
                <div style={{ ...mono, fontSize: 7, color: '#2d4f68' }}>{pct}% load</div>
              </div>
            </div>
          )
        })}
      ...
      {/* Standards note */}
      <div style={{ ...mono, fontSize: 8, color: '#2d4f68', padding: '8px 12px', border: '1px solid #131d28', borderRadius: 3 }}>
...
export default function AutoPlaceModal({
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: '#0d1117', border: '1px solid #1a2b3c', borderRadius: 8,
        ...
        <div style={{
          ...
          padding: '16px 24px', borderBottom: '1px solid #1a2b3c', flexShrink: 0,
        }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {STEPS.map((s, i) => (
                <>
                  ...
                  {i < STEPS.length - 1 && (
                    <div key={`line-${i}`} style={{ width: 28, height: 1, background: step > i+1 ? '#3dba74' : '#1a2b3c' }} />
                  )}
                </>
              ))}
            </div>
          ...
          <button onClick={onClose} style={{
            ...mono, background: 'transparent', border: 'none', color: '#4a7a96', fontSize: 18, cursor: 'pointer', lineHeight: 1,
          }}>✕</button>
        </div>
        ...
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: '14px 24px', borderTop: '1px solid #1a2b3c', flexShrink: 0,
          background: '#080c10',
        }}>
          <div style={{ ...mono, fontSize: 8, color: '#2d4f68' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {step > 1 && (
              <button onClick={handleBack} style={{
                ...
                color: '#4a7a96', border: '1px solid #1a2b3c', borderRadius: 4, cursor: 'pointer', fontSize: 10,
              }}>← BACK</button>
            )}
            <button onClick={onClose} style={{
              ...mono, padding: '8px 18px', background: 'transparent',
            ...
            {step < 3 ? (
              <button
                ...
                style={{
                  ...
                  background: (step === 1 && !spaceId) ? '#131d28' : '#d4a843',
                  color:      (step === 1 && !spaceId) ? '#2d4f68' : '#0f0f0f',
